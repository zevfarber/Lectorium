...
| done (2026-09-19/20, unattended) | 103–110 | 83–90 | `archive/pp083-090.json` | printed = PDF − 20 confirmed (PDF 103 header read "٨٣"); "حكاية القرندلي الثاني" (Second Qalandar) frame continues: the woodcutter's cave/treasure tale, a captive lady kept by an ifrit underground, the porter's-tale-style seduction and punishment scene, then a new tale opens on p. 90 ("قصة المحسود والحاسد", The Envied and the Envier) |
| next | 111–118 | 91–98 (tentative) | | printed = PDF − 20 until told otherwise; verify PDF 111's header before trusting this |
| … | … | … | … | one archive per run |
| end | ~933–938 (offset now −20; recheck near the end) | ~913–920 | | then create `DONE` |

**Last PDF page done: 110 (archive ends at printed p. 90). Pages remaining: ~828 PDF pages
(approximate, pending the corrected offset holding). Lines archived: 1930. Retrain attempted
2026-09-19/20 (pp. 103-110 batch, past the 1800-line trigger): 105 new verified line pairs from
pp. 84, 85, 86, 89, 90 (all five pages clean 1:1 crop-to-line matches, verse/heading lines
excluded; pp. 83, 87, 88 excluded — dropped/merged crops don't map 1:1); lstmtraining ran clean
(1500 iter, train BCER 10.70%) and this time the resulting model was *not worse* on the same two
held-out pages used in prior attempts (printed pp. 40 and 55, never trained on): combined CER
8.80%→8.64%, a small improvement — committed per "never commit a model that made the draft
worse" (this is the first improvement since the pp. 59-66 retrain; the two immediately prior
attempts were discarded as regressions). tools/lines/ now holds pairs for pp. 13-19, 29-32, 43,
45-48, 63-66, 76, 80, 82, 84-86, 89-90. Next retrain attempt whenever the batch composition looks
more favourable (lines archived now 1930, next trigger at 2000).
